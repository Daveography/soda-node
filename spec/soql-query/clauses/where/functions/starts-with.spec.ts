...
describe("StartsWith Where Filter", () => {
  ...
  it("should throw on null column", () => {
    // @ts-ignore TS2345
    const createFunc = () => new StartsWith(null, basicString);
    expect(createFunc).toThrow();
  });

  it("should throw on null value", () => {
    // @ts-ignore TS2345
    const createFunc = () => new StartsWith(new Column("col1"), null);
    expect(createFunc).toThrow();
  });

  it("should throw on empty value", () => {
    const createFunc = () =>
      new StartsWith(new Column("col1"), new WhereValue(""));
    expect(createFunc).toThrow();
  });

  it("should create startswith where filter for basic string value", () => {
    const filterObj = new StartsWith(new Column("col1"), basicString);
    expect(filterObj.toString()).toEqual("starts_with(col1, 'Hello')");
  });
});
