...
describe("In Where Filter", () => {
  ...
  it("should throw on null column", () => {
    // @ts-ignore TS2345
    const createFunc = () => new In(null, stringValues);
    expect(createFunc).toThrow();
  });

  it("should throw on null values", () => {
    // @ts-ignore TS2345
    const createFunc = () => new In(new Column("col1"), null);
    expect(createFunc).toThrow();
  });

  it("should throw on empty values", () => {
    const createFunc = () => new In(new Column("col1"), []);
    expect(createFunc).toThrow();
  });

  it("should create in where filter for string values", () => {
    const filterObj = new In(new Column("col1"), stringValues);
    expect(filterObj.toString()).toEqual("col1 in ('RF1', 'RF2', 'RF3')");
  });

  it("should create in where filter for numeric values", () => {
    const filterObj = new In(new Column("col1"), numericValues);
    expect(filterObj.toString()).toEqual("col1 in ('123', '456', '789')");
  });

  it("should create in where filter for floating timestamp values", () => {
    const filterObj = new In(new Column("col1"), floatingTimestampValues);
    expect(filterObj.toString()).toEqual(
      "col1 in ('1982-04-23T00:00:00.000', '1999-01-15T00:00:00.000', '2012-09-05T00:00:00.000')"
    );
  });
});
