...
describe("Within Circle Where Filter", () => {
  ...
  it("should throw on null column", () => {
    // @ts-ignore TS2345
    const createFunc = () => new WithinCircle(null, loc, 500);
    expect(createFunc).toThrow();
  });

  it("should throw on null point", () => {
    // @ts-ignore TS2345
    const createFunc = () => new WithinCircle(new Column("col1"), null, 500);
    expect(createFunc).toThrow();
  });

  it("should throw on null radius", () => {
    // @ts-ignore TS2345
    const createFunc = () => new WithinCircle(new Column("col1"), loc, null);
    expect(createFunc).toThrow();
  });

  it("should create within_circle where filter with Location", () => {
    const filterObj = new WithinCircle(new Column("col1"), loc, 500);
    expect(filterObj.toString()).toEqual(
      "within_circle(col1, 12.198599, -68.980986, 500)"
    );
  });
  ...
  it("should create within_circle where filter with Point", () => {
    const point: Point = {
      type: "Point",
      coordinates: [-113.50172, 53.528525],
    };
    ...
    expect(filterObj.toString()).toEqual(
      "within_circle(col1, 'POINT (-113.50172 53.528525)', 500)"
    );
  });
});
