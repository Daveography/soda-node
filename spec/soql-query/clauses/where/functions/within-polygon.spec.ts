...
describe("WithinPolygon Where Filter", () => {
  ...
  it("should throw on null column", () => {
    // @ts-ignore TS2345
    const createFunc = () => new WithinPolygon(null, multipolygon);
    expect(createFunc).toThrow();
  });

  it("should throw on null polygon", () => {
    // @ts-ignore TS2345
    const createFunc = () => new WithinPolygon(new Column("col1"), null);
    expect(createFunc).toThrow();
  });

  it("should create within_polygon where filter for multipolygon", () => {
    const filterObj = new WithinPolygon(new Column("col1"), multipolygon);
