...
describe("BasicWhereFilter", () => {
  ...
  it("should throw if query is null", () => {
    const createFunc = () =>
    // @ts-ignore TS2345
      new BasicWhereFilter<ITestInterface, number>(null, column);
  ...
  it("should throw if column is null", () => {
    const createFunc = () =>
    // @ts-ignore TS2345
      new BasicWhereFilter<ITestInterface, any>(query, null);
    expect(createFunc).toThrow();
  });
  ...
  it("should generate simple where equals query", () => {
    ...
    expect(generatedQuery.toString()).toEqual("?$where=id = '1'");
  });
  ...
  it("should generate simple where is not null query", () => {
    ...
    expect(generatedQuery).toEqual("?$where=id IS NOT NULL");
  });
  ...
  it("should generate simple where is null query", () => {
    ...
    expect(generatedQuery).toEqual("?$where=id IS NULL");
  });
  ...
  it("should create grater than where filter with FloatingTimestamp value", () => {
    ...
    expect(generatedQuery).toEqual(
      "?$where=published > '1982-04-23T00:00:00.000'"
    );
  });
});
